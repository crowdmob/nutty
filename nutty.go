...
func New(configFileName *string) *App {
	...
	if err != nil {
		log.Fatalf("Error reading Nuts config: [default].appname %#v\n", err)
	}
	returnedApp.Env, err = config.GetString("default", "env")
	if err != nil {
		log.Fatalf("Error reading Nuts config: [default].env %#v\n", err)
	}
	if len(returnedApp.Env) == 0 {
		returnedApp.Env = "development"
	}
	returnedApp.Logfile, err = config.GetString("default", "logfile")
	if err != nil {
		log.Fatalf("Error reading Nuts config: [default].logfile %#v\n", err)
	}
	returnedApp.Port, err = config.GetInt64("default", "port")
	if err != nil {
		log.Fatalf("Error reading Nuts config: [default].port %#v\n", err)
	}

	// Paypal
	returnedApp.PaypalUsername, err = config.GetString("paypal", "username")
	if err != nil {
		log.Fatalf("Error reading Nuts config: [paypal].username %#v\n", err)
	}
	returnedApp.PaypalPassword, err = config.GetString("paypal", "password")
	if err != nil {
		log.Fatalf("Error reading Nuts config: [paypal].password %#v\n", err)
	}
	returnedApp.PaypalSignature, err = config.GetString("paypal", "signature")
	if err != nil {
		log.Fatalf("Error reading Nuts config: [paypal].signature %#v\n", err)
	}

	// AWS
	awsRegion, err := config.GetString("aws", "region")
	if err != nil {
		log.Fatalf("Error reading Nuts config: [aws].region %#v\n", err)
	}
	if len(awsRegion) == 0 {
		awsRegion = "us-east-1"
	}
	...
	if err != nil {
		log.Fatalf("Error reading Nuts config: [aws].accesskey %#v\n", err)
	}
	awsSecret, err := config.GetString("aws", "secretkey")
	if err != nil {
		log.Fatalf("Error reading Nuts config: [aws].secretkey %#v\n", err)
	}
	...
	if err != nil {
		log.Fatalf("Error reading Nuts config: [sns].arn %#v\n", err)
	}

	// Kafka
	returnedApp.KafkaHost, err = config.GetString("kafka", "host")
	if err != nil {
		log.Fatalf("Error reading Nuts config: [kafka].host %#v\n", err)
	}
	returnedApp.KafkaPort, err = config.GetInt64("kafka", "port")
	if err != nil {
		log.Fatalf("Error reading Nuts config: [kafka].port %#v\n", err)
	}
	...
	if err != nil {
		log.Fatalf("Error reading Nuts config: [kafka].partition %#v\n", err)
	}

	// ServePath
	returnedApp.ServePath, err = config.GetString("default", "servepath")
	if err != nil {
		log.Fatalf("Error reading Nuts config: [default].servepath %#v\n", err)
	}
	if len(returnedApp.ServePath) == 0 {
		returnedApp.ServePath, err = osext.ExecutableFolder()
		if err != nil {
			log.Fatalf("Error setting Nuts Config: osext.ExecutableFolder() %#v\n", err)
		}
	}
